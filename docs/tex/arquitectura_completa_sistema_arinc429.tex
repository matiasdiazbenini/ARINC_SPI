...
La Raspberry Pi 3B+ se encuentra accesible en la red Ethernet directa con IP:

\begin{itemize}
    \item \texttt{192.168.50.2}
\end{itemize}

Esa IP es la usada actualmente por la PC para acceder a Flask. El bridge ARINC queda local dentro de la Raspberry Pi 3B+ y Flask lo consulta por loopback.
...
\midrule
\endhead
Bridge SPI ARINC & 5100 & Consulta a la Pico sniffer por SPI y expone endpoints HTTP locales con estado, snapshot y metricas. \\
Flask dashboard & 5000 & Interfaz web principal de visualizacion operativa del sistema, expuesta hacia la notebook. \\
\bottomrule
\end{longtable}
...
\midrule
\endhead
Prometheus & 9090 & Recolecta metricas expuestas por Flask y permite consultas temporales. \\
...
\begin{itemize}
    \item La 3B+ mantiene el bridge local en \texttt{http://127.0.0.1:5100}.
    \item La 3B+ expone el dashboard Flask en \texttt{http://192.168.50.2:5000}.
    \item La PC ejecuta Prometheus en \texttt{http://127.0.0.1:9090}.
    \item La PC ejecuta Grafana en \texttt{http://127.0.0.1:3000}.
    \item Prometheus scrapea Flask en \texttt{192.168.50.2:5000/metrics}.
\end{itemize}
...
\begin{itemize}
    \item La 3B+ se identifica en la red Ethernet directa como \texttt{192.168.50.2}.
    \item El bridge HTTP escucha localmente en \texttt{127.0.0.1:5100}.
    \item Flask escucha hacia la red en \texttt{192.168.50.2:5000}.
...
\begin{tikzpicture}[x=1cm,y=1cm,>=latex]
    ...
    \draw[thick] (prom) -- node[above]{scrape /metrics\\192.168.50.2:5000} (pi);
...
\begin{itemize}
    \item Raspberry Pi 3B+: \texttt{192.168.50.2}
    \item Bridge SPI HTTP: \texttt{127.0.0.1:5100}
    \item Dashboard Flask: \texttt{192.168.50.2:5000}
...
La direccion \texttt{192.168.50.2} debe entenderse como configuracion recomendada de la 3B+ en la red Ethernet directa de laboratorio. Si la IP cambia, deben ajustarse:
